...
- `-r, --rounding` - Стратегия округления: `mx` (максимальное), `mn` (минимальное), `nr` (ближайшее). По умолчанию: mx. Внутри использует enum `RoundingStrategy`
...
*Необязательный второй аргумент после папки — фильтр файлов: расширение (`.tif`), окончание имени (`RAW.tif`) или глоб (`"*.RAW.tif"`). В bash глоб берите в кавычки, иначе его раскроет оболочка.*
