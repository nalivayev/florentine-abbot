...
Проект в некоторой степени реализует подходы эталонной модели **OAIS (Open Archival Information System)** (ISO 14721) — стандарта для долгосрочного хранения данных, используемого архивами и библиотеками, например в NASA, где, собственно, и был разработан. Для оцифровки и качества изображения он также опирается на идеи рекомендаций **FADGI** (Federal Agencies Digital Guidelines Initiative), широко применяемых в учреждениях культурного наследия.

Это попытка применения ключевых концепций OAIS для задач ведения личного архива. В частности:
...
- **Access / Доступ**: Создание компонента доступа. Архивы не выдают пользователям оригинальные мастер-файлы, чтобы избежать их порчи или утраты. Вместо этого создаются "пользовательские копии" (превью, PDF, облегченные JPEG).
