...
Florentine Abbot в некоторой степени реализует подходы эталонной модели **OAIS (Open Archival Information System)** (ISO 14721) — стандарта для долгосрочного хранения данных, используемого национальными архивами и библиотеками (NASA, Библиотека Конгресса).

Система адаптирует ключевые концепции OAIS для задач личного архива:

- **Ingest / Прием (`Scan Batcher`)**: Процесс получения данных, контроля качества (валидации) и подготовки к хранению. Модуль сканирует, проверяет и формирует пакеты данных.
- **Archival Storage / Архивное хранение (`File Organizer`)**: Компонент долгосрочного хранения. Он перемещает данные в хранилище "Мастеров" (`2_Masters`), присваивая неизменяемые идентификаторы и создавая **Archival Information Packages (AIP)**.
- **Access / Доступ (`Preview Maker`)**: Компонент доступа. Архивы не выдают пользователям оригинальные мастер-файлы, чтобы избежать их порчи или утраты. Вместо этого создаются "пользовательские копии" (превью, PDF, облегченные JPEG). Этот модуль генерирует **Dissemination Information Packages (DIP)**.

Такая архитектура гарантирует, что "Мастеры" остаются нетронутыми и безопасными, обеспечивая при этом удобный доступ через легкие производные копии.
