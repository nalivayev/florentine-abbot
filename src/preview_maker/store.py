"""Database boundary for Preview Maker task orchestration."""
...
class MakerTaskRecord:
    """Minimal task candidate state needed by Preview Maker orchestration."""
...
class MakerStore:
    """Thin package-local wrapper around shared preview-maker task state."""
    ...
    def _c(self) -> sqlite3.Connection:
        if self._conn is None:
            raise RuntimeError("MakerStore is not open")
        return self._conn

    def _commit(self) -> None:
        self._c.commit()

    def list_pending_files(self) -> list[MakerTaskRecord]:
