...
class TestMetadataFlow:
    ...
    def test_organizer_to_maker_metadata_consistency(self, tmp_path: Path) -> None:
        ...
        assert self._is_uuid(master_tags[TAG_XMP_DC_IDENTIFIER])
        assert self._is_uuid(master_tags[TAG_XMP_XMP_IDENTIFIER])
        assert self._is_uuid(prv_tags[TAG_XMP_DC_IDENTIFIER])
        assert self._is_uuid(prv_tags[TAG_XMP_XMP_IDENTIFIER])
        assert master_tags[TAG_XMP_DC_IDENTIFIER] != prv_tags[TAG_XMP_DC_IDENTIFIER]
